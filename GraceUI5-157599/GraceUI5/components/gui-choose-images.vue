			...
			uni.chooseMedia({
				...
				success    : (res) => {
					...
					for(let i = 0; i < res.tempFiles.length; i++){
						if(res.tempFiles[i].size > this.maxFileSize * (1024 * 1024)){
							uni.showToast({
								title:"图片尺寸过大，已忽略",
								icon:"none"
							});
							...
							this.imgLists.push({url:res.tempFiles[i].tempFilePath, progress:0});
						}
					}
			...
			uni.chooseImage({
			    ...
			    success    : (res) => {
					...
					for(let i = 0; i < res.tempFilePaths.length; i++){
						if(this.imgLists.length >= this.maxFileNumber){break;}
						this.imgLists.push({url:res.tempFilePaths[i], progress:0, data:res.tempFiles[i]});
					}
			        this.$emit('change', this.imgLists);
			    },
				complete   : function(){}
